...
public class OldUsersConverter {
    ...
    public static boolean convertUserBanlist(final MinecraftServer minecraftserver) {
        ...
        if (OldUsersConverter.OLD_USERBANLIST.exists() && OldUsersConverter.OLD_USERBANLIST.isFile()) {
            ...
            try {
                ...
                readOldListFormat(OldUsersConverter.OLD_USERBANLIST, (Map) map);
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(GameProfile gameprofile) {
                        ...
                        if (astring == null) {
                            ...
                            throw new OldUsersConverter.FileConversionException("Profile not in the conversionlist");
                        } else {
                    ...
                    public void onProfileLookupFailed(GameProfile gameprofile, Exception exception) {
                        ...
                        if (!(exception instanceof ProfileNotFoundException)) {
                            throw new OldUsersConverter.FileConversionException("Could not request user " + gameprofile.getName() + " from backend systems", exception);
                        }
                    }
                };
                ...
                return false;
            } catch (OldUsersConverter.FileConversionException namereferencingfileconverter_fileconversionexception) {
    ...
    public static boolean convertIpBanlist(MinecraftServer minecraftserver) {
        ...
        if (OldUsersConverter.OLD_IPBANLIST.exists() && OldUsersConverter.OLD_IPBANLIST.isFile()) {
            ...
            try {
                ...
                readOldListFormat(OldUsersConverter.OLD_IPBANLIST, (Map) map);
        ...
        if (OldUsersConverter.OLD_OPLIST.exists() && OldUsersConverter.OLD_OPLIST.isFile()) {
            ...
            try {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(GameProfile gameprofile) {
                        minecraftserver.getProfileCache().add(gameprofile);
                        oplist.add(new ServerOpListEntry(gameprofile, minecraftserver.getOperatorUserPermissionLevel(), false));
                    }
    ...
    public static UUID convertMobOwnerIfNecessary(final MinecraftServer minecraftserver, String s) {
        if (!StringUtil.isNullOrEmpty(s) && s.length() <= 16) {
            GameProfile gameprofile = minecraftserver.getProfileCache().get(s);
            ...
            if (gameprofile != null && gameprofile.getId() != null) {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(GameProfile gameprofile1) {
                        minecraftserver.getProfileCache().add(gameprofile1);
                        list.add(gameprofile1);
                    }

                    public void onProfileLookupFailed(GameProfile gameprofile1, Exception exception) {
                        OldUsersConverter.LOGGER.warn("Could not lookup user whitelist entry for {}", gameprofile1.getName(), exception);
                    }
                };
                ...
                return Player.createPlayerUUID(new GameProfile((UUID) null, s));
            }
        } else {
            try {
            ...
            try {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    public void onProfileLookupSucceeded(GameProfile gameprofile) {
                        ...
                        if (uuid == null) {
                            throw new OldUsersConverter.FileConversionException("Missing UUID for user profile " + gameprofile.getName());
                        } else {
                    ...
                    private void movePlayerFile(File file4, String s2, String s3) {
                        ...
                        if (!file5.renameTo(file6)) {
                            throw new OldUsersConverter.FileConversionException("Could not convert file for " + s2);
                        }
                    }
                    ...
                    private String getFileNameForProfile(GameProfile gameprofile) {
                        ...
                        if (s2 == null) {
                            throw new OldUsersConverter.FileConversionException("Could not find the filename for " + gameprofile.getName() + " anymore");
                        } else {
                            return s2;
                        }
                    }
    ...
    private static void ensureDirectoryExists(File file) {
        if (file.exists()) {
            if (!file.isDirectory()) {
                throw new OldUsersConverter.FileConversionException("Can't create directory " + file.getName() + " in world save directory.");
            }
        } else if (!file.mkdirs()) {
            throw new OldUsersConverter.FileConversionException("Can't create directory " + file.getName() + " in world save directory.");
        }
