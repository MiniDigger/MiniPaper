...
public class Ravager extends Raider {
    ...
    protected void updateControlFlags() {
        boolean flag = !(this.getControllingPassenger() instanceof Mob) || this.getControllingPassenger().getType().is((Tag) EntityTypeTags.RAIDERS);
    ...
    public static AttributeSupplier.Builder m() {
        return Monster.eS().a(Attributes.MAX_HEALTH, 100.0D).a(Attributes.MOVEMENT_SPEED, 0.3D).a(Attributes.KNOCKBACK_RESISTANCE, 0.75D).a(Attributes.ATTACK_DAMAGE, 12.0D).a(Attributes.ATTACK_KNOCKBACK, 1.5D).a(Attributes.FOLLOW_RANGE, 32.0D);
    }
        ...
        if (this.isAlive()) {
            ...
            if (this.horizontalCollision && this.level.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (block instanceof LeavesBlock && !org.bukkit.craftbukkit.event.CraftEventFactory.callEntityChangeBlockEvent(this, blockposition, Blocks.AIR.getBlockData()).isCancelled()) { // CraftBukkit
    ...
    protected void blockedByShield(LivingEntity entityliving) {
        if (this.roarTick == 0) {
            if (this.random.nextDouble() < 0.5D) {
                ...
                this.strongKnockback((Entity) entityliving);
            }
    ...
    private void roar() {
        if (this.isAlive()) {
            List<Entity> list = this.level.getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(4.0D), Ravager.NO_RAVAGER_AND_ALIVE);

            Entity entity;

            for (Iterator iterator = list.iterator(); iterator.hasNext(); this.strongKnockback(entity)) {
    ...
    static class RavagerNodeEvaluator extends WalkNodeEvaluator {
        ...
        @Override
        protected BlockPathTypes evaluateBlockPathType(BlockGetter iblockaccess, boolean flag, boolean flag1, BlockPos blockposition, BlockPathTypes pathtype) {
            return pathtype == BlockPathTypes.LEAVES ? BlockPathTypes.OPEN : super.evaluateBlockPathType(iblockaccess, flag, flag1, blockposition, pathtype);
        }
    }

    static class RavagerNavigation extends GroundPathNavigation {

        public RavagerNavigation(Mob entityinsentient, Level world) {
