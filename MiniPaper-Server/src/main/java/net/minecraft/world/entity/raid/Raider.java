...
public abstract class Raider extends PatrollingMonster {
    ...
    protected Raider(EntityType<? extends Raider> entitytypes, Level world) {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.register(Raider.IS_CELEBRATING, false);
    }
    ...
    public void readAdditionalSaveData(CompoundTag nbttagcompound) {
        ...
        if (nbttagcompound.contains("RaidId", 3)) {
            if (this.level instanceof ServerLevel) {
                this.raid = ((ServerLevel) this.level).getRaids().get(nbttagcompound.getInt("RaidId"));
            }
    ...
    public SpawnGroupData prepare(LevelAccessor generatoraccess, DifficultyInstance difficultydamagescaler, MobSpawnType enummobspawn, @Nullable SpawnGroupData groupdataentity, @Nullable CompoundTag nbttagcompound) {
        this.setCanJoinRaid(this.getType() != EntityType.WITCH || enummobspawn != MobSpawnType.NATURAL);
    ...
    static class RaiderMoveThroughVillageGoal extends Goal {
        ...
        public RaiderMoveThroughVillageGoal(Raider entityraider, double d0, int i) {
        ...
        public void tick() {
            if (this.raider.getNavigation().isDone()) {
                Vec3 vec3d = Vec3.atBottomCenterOf((Vec3i) this.poiPos);
                Vec3 vec3d1 = RandomPos.getPosTowards(this.raider, 16, 7, vec3d, 0.3141592741012573D);

                if (vec3d1 == null) {
                    vec3d1 = RandomPos.getPosTowards(this.raider, 8, 7, vec3d);
                }
        ...
        private boolean hasNotVisited(BlockPos blockposition) {
    ...
    public class HoldGroundAttackGoal extends Goal {
        ...
        public void start() {
            ...
            while (iterator.hasNext()) {
                ...
                entityraider.setGoalTarget(this.mob.getTarget(), org.bukkit.event.entity.EntityTargetEvent.TargetReason.FOLLOW_LEADER, true); // CraftBukkit
            }
        ...
        public void tick() {
            ...
            if (entityliving != null) {
                if (this.mob.distanceToSqr((Entity) entityliving) > (double) this.hostileRadiusSqr) {
                    this.mob.getControllerLook().setLookAt(entityliving, 30.0F, 30.0F);
