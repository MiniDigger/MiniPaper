...
public class CraftChunk implements Chunk {
    ...
    public ChunkSnapshot getChunkSnapshot(boolean includeMaxBlockY, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        for (int i = 0; i < cs.length; i++) {
            if (cs[i] == null) { // Section is empty?
                ...
                PalettedContainer blockids = new PalettedContainer<>(LevelChunkSection.GLOBAL_BLOCKSTATE_PALETTE, net.minecraft.world.level.block.Block.BLOCK_STATE_REGISTRY, NbtUtils::readBlockState, NbtUtils::writeBlockState, Blocks.AIR.getBlockData()); // TODO: snapshot whole ChunkSection
                blockids.read(data.getList("Palette", CraftMagicNumbers.NBT.TAG_COMPOUND), data.getLongArray("BlockStates"));

                sectionBlockIDs[i] = blockids;

                LevelLightEngine lightengine = chunk.level.getChunkSourceOH().getLightEngine();
        ...
        if (includeMaxBlockY) {
            hmap = new Heightmap(null, Heightmap.Types.MOTION_BLOCKING);
            hmap.setRawData(chunk.heightmaps.get(Heightmap.Types.MOTION_BLOCKING).getRawData());
        }

        ChunkBiomeContainer biome = null;

        if (includeBiome || includeBiomeTempRain) {
            biome = chunk.getBiomeIndex().copy();
        }
    ...
    public static ChunkSnapshot getEmptyChunkSnapshot(int x, int z, CraftWorld world, boolean includeBiome, boolean includeBiomeTempRain) {
        ...
        if (includeBiome || includeBiomeTempRain) {
            BiomeSource wcm = world.getHandle().getChunkSourceOH().getGenerator().getWorldChunkManager();
            biome = new ChunkBiomeContainer(new ChunkPos(x, z), wcm);
        }
        ...
        return new CraftChunkSnapshot(x, z, world.getName(), world.getFullTime(), blockIDs, skyLight, emitLight, empty, new Heightmap(null, Heightmap.Types.MOTION_BLOCKING), biome);
    }
