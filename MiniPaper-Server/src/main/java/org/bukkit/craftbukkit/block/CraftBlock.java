...
public class CraftBlock implements Block {
    ...
    public World getWorld() {
        return world.getLevel().getWorld();
    }
    ...
    @Override
    public byte getData() {
        net.minecraft.world.level.block.state.BlockState blockData = world.getType(position);
    ...
    public static Biome biomeBaseToBiome(net.minecraft.world.level.biome.Biome base) {
        if (base == null) {
            return null;
        }
    ...
    public static net.minecraft.world.level.biome.Biome biomeToBiomeBase(Biome bio) {
        if (bio == null) {
            return null;
        }
    ...
    public boolean isBlockPowered() {
        return world.getLevel().getDirectSignalTo(position) > 0;
    }
    ...
    public int getBlockPower(BlockFace face) {
        int power = 0;
        net.minecraft.world.level.Level world = this.world.getLevel();
        int x = getX();
        int y = getY();
        int z = getZ();
        if ((face == BlockFace.DOWN || face == BlockFace.SELF) && world.hasSignal(new BlockPos(x, y - 1, z), Direction.DOWN)) power = getPower(power, world.getType(new BlockPos(x, y - 1, z)));
        if ((face == BlockFace.UP || face == BlockFace.SELF) && world.hasSignal(new BlockPos(x, y + 1, z), Direction.UP)) power = getPower(power, world.getType(new BlockPos(x, y + 1, z)));
        if ((face == BlockFace.EAST || face == BlockFace.SELF) && world.hasSignal(new BlockPos(x + 1, y, z), Direction.EAST)) power = getPower(power, world.getType(new BlockPos(x + 1, y, z)));
        if ((face == BlockFace.WEST || face == BlockFace.SELF) && world.hasSignal(new BlockPos(x - 1, y, z), Direction.WEST)) power = getPower(power, world.getType(new BlockPos(x - 1, y, z)));
        if ((face == BlockFace.NORTH || face == BlockFace.SELF) && world.hasSignal(new BlockPos(x, y, z - 1), Direction.NORTH)) power = getPower(power, world.getType(new BlockPos(x, y, z - 1)));
        if ((face == BlockFace.SOUTH || face == BlockFace.SELF) && world.hasSignal(new BlockPos(x, y, z + 1), Direction.SOUTH)) power = getPower(power, world.getType(new BlockPos(x, y, z + 1)));
    ...
    public boolean breakNaturally(ItemStack item) {
        ...
        if (block != Blocks.AIR && (item == null || iblockdata.requiresCorrectToolForDrops() || nmsItem.canDestroySpecialBlock(iblockdata))) {
            net.minecraft.world.level.block.Block.dropItems(iblockdata, world.getLevel(), position, world.getBlockEntity(position), null, nmsItem);
            result = true;
        }
    ...
    public Collection<ItemStack> getDrops(ItemStack item, Entity entity) {
        ...
        // Modelled off EntityHuman#hasBlock
        if (item == null || iblockdata.requiresCorrectToolForDrops() || nms.canDestroySpecialBlock(iblockdata)) {
            return net.minecraft.world.level.block.Block.getDrops(iblockdata, (ServerLevel) world.getLevel(), position, world.getBlockEntity(position), entity == null ? null : ((CraftEntity) entity).getHandle(), nms)
